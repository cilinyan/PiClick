...
from isegm.model.losses_ranking import DETRLikeWithRankingLoss
from isegm.model.is_multimask_with_ranking_model import MultiMaskWithRankingModel
from isegm.engine.trainer_rank import ISTrainerRank

MODEL_NAME = 'multimask_ranking_base448_cocolvis_itermask_no_ranking_loss'
...
def train(model, cfg, model_cfg):
    ...
    loss_cfg = edict()
    loss_cfg.instance_loss = DETRLikeWithRankingLoss(
        num_queries=_PARAMS['num_queries'],
        num_classes=_PARAMS['num_classes'],
        loss_rank=dict(type='L1Loss', loss_weight=0.0, ),
    )
    ...
    trainer = ISTrainerRank(model, cfg, model_cfg, loss_cfg,
                            trainset, valset,
                            optimizer='adam',
