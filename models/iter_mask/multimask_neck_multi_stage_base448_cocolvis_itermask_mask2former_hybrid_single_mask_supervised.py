...
from isegm.model.is_multimask_neck_multi_stage_model_mask2former_hybrid import \
    MultiMaskNeckMultiStageModelMask2FormerHybrid
from isegm.engine.trainer_single import ISTrainerSingle

MODEL_NAME = 'multimask_neck_multi_stage_base448_cocolvis_itermask_mask2former_hybrid_single_mask_supervised'
...
def init_model(cfg):
    ...
    model = MultiMaskNeckMultiStageModelMask2FormerHybrid(
        num_classes=_PARAMS['num_classes'],
        num_queries=_PARAMS['num_queries'],
        use_disks=True,
        norm_radius=5,
...
def train(model, cfg, model_cfg):
    ...
    loss_cfg = edict()
    loss_cfg.instance_loss = DETRLikeLoss(num_queries=_PARAMS['num_queries'],
                                          num_classes=_PARAMS['num_classes'])
    ...
    lr_scheduler = partial(torch.optim.lr_scheduler.MultiStepLR, milestones=[50, 65], gamma=0.1)
    trainer = ISTrainerSingle(model, cfg, model_cfg, loss_cfg,
                              trainset, valset,
                              optimizer='adam',
    ...
    trainer.run(num_epochs=80, validation=False)
