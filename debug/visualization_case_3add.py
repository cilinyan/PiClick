...
def main():
    ...
    for i, case in enumerate(_IMAGE_PAIRS):
        ...
        for idx_mask, thr in case['indices']:
            ...
            if (case['piclick'] == 'debug/vis/vis_in/193_0_n3.pkl') and (idx_mask == 2):
                mask_pci = (np.array(data_pc['pred_probs'][idx_mask] > thr, dtype=float) * 255).astype(np.uint8)
                w_ = int(mask_pci.shape[1] * 374 / 759)
                mask_pci[:, :w_] = \
                    (np.array(data_pc['pred_probs'][idx_mask][:, :w_] > 0.47, dtype=float) * 255).astype(np.uint8)
    ...
    save_image_split(images, len(images) // col, col, out_dir='debug/vis/vis_in/split')
...
def save_image_split(images, row, col, out_dir):
    ...
    for idx, img in enumerate(images):
        ...
        # save image
        cv2.imwrite(os.path.join(out_dir, 'img_{}_{}.jpg'.format(r, c)), img)
    pass
