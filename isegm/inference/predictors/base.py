...
class BasePredictor(object):
    def __init__(self, model, device,
        ...
        self.mask_dir = osp.join('/data/clyan/quicksilver/temp_data', str(time.time()).replace('.', '_'))
    ...
    # @logger.catch()
    def flipped_mask_match(self, cls_scores, masks_pred, seg_thr=.49, max_score: bool = False, vis_flag: bool = False):
        """
        Args:
        ...
        scores_sum = score_0 + score_1
        idx_max_scores = torch.argmax(scores_sum).item()
        if max_score:
            # score_0 = score_0[idx_max_scores: idx_max_scores + 1]
            # score_1 = score_1[idx_max_scores: idx_max_scores + 1]
            mask_0 = mask_0[idx_max_scores: idx_max_scores + 1]
            mask_1 = mask_1[idx_max_scores: idx_max_scores + 1]
            mask_0_p = mask_0_p[idx_max_scores: idx_max_scores + 1]
            mask_1_p = mask_1_p[idx_max_scores: idx_max_scores + 1]
        else:
            idx_keep = [i for i, s in enumerate(scores_sum) if s > seg_thr]
            idx_keep = list(set(idx_keep + [idx_max_scores]))
            # pdb.set_trace()
            mask_0 = mask_0[idx_keep]
            mask_1 = mask_1[idx_keep]
            mask_0_p = mask_0_p[idx_keep]
            mask_1_p = mask_1_p[idx_keep]
    ...
    def get_prediction(self, clicker, prev_mask=None, output_tuple=False, **kwargs):
        ...
        if output_tuple:
            ...
            instances = self._get_prediction_tuple(image_nd, clicks_lists, is_image_changed)
            pred_logits, pred_rignt_ = self.flipped_mask_match(*instances, max_score=not self.max_iou)
        elif not self.max_iou:  # SimpleClick 测试模式
            pred_logits = self._get_prediction(image_nd, clicks_lists, is_image_changed)
            pred_rignt_ = None
        else:  # 输出所有可能的 masks
            pred_logits = self._get_prediction_max_iou(image_nd, clicks_lists, is_image_changed, **kwargs)
            pred_rignt_ = None
        # import pdb; pdb.set_trace()
        ...
        # pdb.set_trace()
        # prediction = self.transforms[0].inv_transform(prediction, save_delay=True)
        # prediction = self.transforms[1].inv_transform(prediction)
        for t in reversed(self.transforms):
            if isinstance(t, ZoomIn) and self.max_iou:
        ...
        #     max_iou, idx_max_iou, mask_max_iou, prob_max_iou = \
        #         select_max_iou_mask(gt_mask, prediction.cpu().numpy()[0], 0.49)
        #     test_data = mask_max_iou[None, None, :, :]
        # except:
        #     pdb.set_trace()

        if not self.max_iou:  # SimpleClick 测试模式
            self.prev_prediction = prediction
            return prediction.cpu().numpy()[0, 0]
        else:
            return prediction.cpu().numpy()[0]

    def _get_prediction_tuple(self, image_nd, clicks_lists, is_image_changed, **kwargs):
        points_nd = self.get_points_nd(clicks_lists)
        # import pdb; pdb.set_trace()
        return self.net(image_nd, points_nd, last_layer=True)['instances'][:2]

    def _get_prediction_max_iou(self, image_nd, clicks_lists, is_image_changed, **kwargs):
        points_nd = self.get_points_nd(clicks_lists)
        # import pdb; pdb.set_trace()
        return self.net(image_nd, points_nd, max_iou=True, **kwargs)['instances']
