...
def output_batch_no_first(output):
    output['instances'] = (i.transpose(0, 1) for i in output['instances'])
...
class ISTrainerMulti(object):
    ...
    def batch_forward(self, batch_data, validation=False):
        ...
        with torch.set_grad_enabled(not validation):
            ...
            prev_output = torch.zeros(b, self.num_queries, h, w).to(image)
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    prev_output = torch.sigmoid(output['masks_sorted'])
                    masks_choice = torch.sigmoid(output['masks_choice'])

                    logger.debug('masks_sorted: {}'.format(prev_output.shape))
                    ...
                    points = get_next_points(masks_choice, orig_gt_mask, points, click_indx + 1)
            ...
            single_gt_masks = [m.long().to(self.device) for m in gt_mask]

            net_input = torch.cat((image, prev_output), dim=1) if self.net.module.with_prev_mask else image
            output = self.net(net_input, points, batch_first=True, train_mode=True)
            output = output_batch_no_first(output)

            # ls = [name for name, para in self.net.named_parameters() if para.grad is None]
            # logger.error(ls)

            loss = 0.0
            loss = self.add_loss('instance_loss', loss, losses_logging, validation,
                                 lambda: (output['instances'], gt_masks, single_gt_masks))
