        ...
        with torch.set_grad_enabled(not validation):
            ...
            with torch.no_grad():
                ...
                for click_indx in range(num_iters):
                    ...
                    output = eval_model.module(net_input, points, test_mode=True)['instances']
                    logger.debug(f'ITER OUTPUT SHAPE: {output.shape}')
            ...
            gt_masks = [m.long().to(self.device) for m in gt_mask]
            logger.debug(f'GT_MASKS: {gt_masks[0].shape}')
