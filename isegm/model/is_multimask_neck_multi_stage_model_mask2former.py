...
from .modeling.mask2former_helper.mask2former_head import Mask2FormerHead
...
def select_all_possible(cls_scores_list, mask_preds_list, batch_first: bool = False, cls_thr=0.49):
    # 仅在单图推理中使用
    if batch_first:  # B, NUM_LAYER, NUM_QUERY, NUM_CLASS -> B, NUM_QUERY, NUM_CLASS
        ...
        cls_scores_list = cls_scores_list[-1]
        mask_preds_list = mask_preds_list[-1]
    cls_scores_list = cls_scores_list.softmax(dim=-1)[0, :, 0]  # NUM_QUERY
    mask_preds_list = mask_preds_list[0]  # NUM_QUERY, H, W
    idx_max_score = torch.argmax(cls_scores_list, dim=-1).item()
    idx_possible = torch.where(cls_scores_list > cls_thr)[0].cpu().numpy().tolist()
...
class MultiMaskNeckMultiStageModelMask2Former(ISModel):
    @serialize
    def __init__(
            self,
        ...
        self.head = Mask2FormerHead(**head_params)
        ...
        self.importance_sample_ratio = TRAIN_CFG.get('importance_sample_ratio', 0.75)
    ...
    def forward(self, image, points, batch_first=False, **kwargs):
        ...
        if kwargs.get('max_iou', False):
            cls_thr = kwargs.get('cls_thr', 0.49)
            outputs['instances'] = select_all_possible(*outputs['instances'], batch_first=batch_first, cls_thr=cls_thr)
    ...
    def _get_target_single(self,
        ...
        # logger.error(f'cls_score: {cls_score.shape}, {cls_score.dtype}, {cls_score.device}')
        # logger.error(f'mask_points_pred: {mask_points_pred.shape}, {mask_points_pred.dtype}, {mask_points_pred.device}')
        # logger.error(f'gt_labels: {gt_labels.shape}, {gt_labels.dtype}, {gt_labels.device}')
        # logger.error(f'gt_points_masks: {gt_points_masks.shape}, {gt_points_masks.dtype}, {gt_points_masks.device}')

        # assign and sample
        assign_result = self.assigner.assign(cls_score, mask_points_pred, gt_labels.long(), gt_points_masks, )
