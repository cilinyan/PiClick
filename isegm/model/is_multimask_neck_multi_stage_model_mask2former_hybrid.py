...
def select_last_layer(cls_scores_list, mask_preds_list, batch_first: bool = False):
    # 仅在单图推理中使用
    if batch_first:  # B, NUM_LAYER, NUM_QUERY, NUM_CLASS -> B, NUM_QUERY, NUM_CLASS
        cls_scores_list = cls_scores_list[:, -1, ...]
        mask_preds_list = mask_preds_list[:, -1, ...]
        ...
        cls_scores_list = cls_scores_list[-1]
        mask_preds_list = mask_preds_list[-1]
    return cls_scores_list, mask_preds_list  # 1, NUM_SELECT, H, W
...
class MultiMaskNeckMultiStageModelMask2FormerHybrid(ISModel):
    @serialize
    def __init__(
            self,
