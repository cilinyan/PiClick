...
class DETRLikeWithRankingLoss(nn.Module):
    def __init__(self,
                 num_queries: int,
                 num_classes: int,
                 loss_cls: dict = dict(
        ...
        super(DETRLikeWithRankingLoss, self).__init__()
    ...
    def loss(self, all_cls_scores, all_mask_preds, all_rank_scores, gt_labels_list, gt_masks_list, single_gt_masks):
        """Loss function.
        ...
        losses_cls, losses_mask, losses_dice, loss_rank = multi_apply(self.loss_single,
                                                                      all_cls_scores,
                                                                      all_mask_preds,
                                                                      all_rank_scores,
                                                                      all_gt_labels_list,
                                                                      all_gt_masks_list,
                                                                      all_gt_single_gt_masks_list,
                                                                      )
        ...
        num_dec_layer = 0
        for loss_cls_i, loss_mask_i, loss_dice_i, loss_rank_i in zip(losses_cls[:-1],
                                                                     losses_mask[:-1],
                                                                     losses_dice[:-1],
                                                                     loss_rank[:-1]):
    ...
    def loss_single(self, cls_scores, mask_preds, rank_scores, gt_labels_list, gt_masks_list, single_gt_masks):
        ...
        return loss_cls, loss_mask, loss_dice, loss_rank
    ...
    def forward(self,
        ...
        """
        all_cls_scores, all_mask_preds, all_rank_scores = outputs
        device = gt_masks[0].device
        gt_labels = [
            torch.tensor([0] * m.shape[0], dtype=torch.int).long().to(device) for m in gt_masks
        ]
        losses = self.loss(all_cls_scores, all_mask_preds, all_rank_scores, gt_labels, gt_masks, single_gt_masks)
