...
def select_last_layer(cls_scores_list, mask_preds_list, rank_scores_list, batch_first: bool = False):
    # 仅在单图推理中使用
    if batch_first:  # B, NUM_LAYER, NUM_QUERY, NUM_CLASS -> B, NUM_QUERY, NUM_CLASS
        cls_scores_list = cls_scores_list[:, -1, ...]
        mask_preds_list = mask_preds_list[:, -1, ...]
...
class MultiMaskWithRankingModel(ISModel):
    ...
    def forward(self, image, points, batch_first=False, **kwargs):
        ...
        query_points, points_attn_mask = self.get_batch_click_embedding(points)
    ...
    def get_single_click_embedding(self, x_in: torch.Tensor):
        # x (Tensor): shape [24, 2]
        x = x_in.long()
    ...
    def get_batch_click_embedding(self, x):
        x_emb, points_attn_mask = list(zip(*[self.get_single_click_embedding(xi) for xi in x]))
