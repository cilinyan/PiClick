...
from .modeling.mask2former_helper.mask2former_head import ClickAddMaskSimple2FormerHead
...
def select_last_layer(cls_scores_list, mask_preds_list, rank_scores_list, alpha, batch_first: bool = False):
    # mask_preds_list: NUM_LAYER, B, NUM_QUERY, H, W, torch.Size([10, 2, 7, 448, 448])
    # cls_scores_list: NUM_LAYER, B, NUM_QUERY, NUM_CLASS, torch.Size([10, 2, 7, 2])
    # rank_scores_list: NUM_LAYER, B, NUM_QUERY, 1, torch.Size([10, 2, 7, 1])
    # 仅在单图推理中使用
    ...
    scores = (1 - alpha) * cls_scores_list + alpha * rank_scores_list
    # return cls_scores_list, mask_preds_list, rank_scores_list  # 1, NUM_SELECT, H, W
...
def select_max_score_mask(cls_scores_list,
    ...
    # logger.error(f'OUTPUT SHAPE: {max_scores_masks.shape}')
...
class MultiMaskWithRankingModel(ISModel):
    @serialize
    def __init__(
            self,
        ...
        self.head = ClickAddMaskSimple2FormerHead(**head_params)
    ...
    def forward(self, image, points, batch_first=False, **kwargs):
        ...
        if not isinstance(outputs['instances'], tuple):
            ...
            all_cls_scores, all_mask_preds, all_iou_preds = outputs['instances']
            ...
            if batch_first:
                outputs['instances'] = \
                    all_cls_scores.transpose(0, 1), all_mask_preds.transpose(0, 1), all_iou_preds.transpose(0, 1)
            else:
                outputs['instances'] = all_cls_scores, all_mask_preds, all_iou_preds
