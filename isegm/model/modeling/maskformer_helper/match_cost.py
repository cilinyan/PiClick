...
class DistCost:
    ...
    def __call__(self, cls_pred, gt_labels):
        return self.weight * torch.cdist(cls_pred[:, None], gt_labels[:, None], p=2)
...
class ClassificationCost:
    ...
    def __call__(self, cls_pred, gt_labels):
        ...
        #     # gt_labels = gt_labels.clone().detach().long()
        #     cls_cost = -cls_score[:, gt_labels]
        # except Exception as e:
        #     logger.error(f'ERROR: {e}')
        #     logger.error(f'cls_score: {cls_pred}, {cls_pred.dtype}, {cls_pred.shape}')
        #     logger.error(f'cls_score: {cls_score}, {cls_score.dtype}, {cls_score.shape}')
        #     logger.error(f'gt_labels: {gt_labels}, {gt_labels.dtype}, {gt_labels.shape}')
        #     raise
        return cls_cost * self.weight
...
class DiceCost:
    ...
    def __init__(self, weight=1., pred_act=False, eps=1e-3, naive_dice=True, **kwargs):
        self.weight = weight
...
class CrossEntropyLossCost:
    ...
    def __init__(self, weight=1., use_sigmoid=True, **kwargs):
