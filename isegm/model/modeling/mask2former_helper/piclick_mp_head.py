...
class PiClickMPHead(nn.Module):
    ...
    def forward(self, feats, ):
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            attn_masks = [torch.cat([attn_mask, ], dim=2), None]

            query_feat = layer(
                query=query_feat,
                key=torch.cat([decoder_inputs[level_idx], ], dim=0),
                value=torch.cat([decoder_inputs[level_idx], ], dim=0),
                query_pos=query_embed,
                key_pos=torch.cat([decoder_positional_encodings[level_idx], ], dim=0),
