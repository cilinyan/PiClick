...
class ClickDespair2FormerHead(nn.Module):
    ...
    def __init__(self,
        ...
        self.onehot_embed = nn.Sequential(
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, feat_channels), nn.ReLU(inplace=True),
            nn.Linear(feat_channels, 1))
    ...
    def forward_head(self, decoder_out, mask_feature, attn_mask_target_size):
        ...
        onehot_pred = self.onehot_embed(decoder_out)
        # shape (batch_size, num_queries, c)
        ...
        return cls_pred, mask_pred, attn_mask, iou_pred, onehot_pred

    def forward(self, feats, query_points, points_invalid):
        """Forward function.
        ...
        Args:
            ...
            invalid (Tensor): [batch_size, num_points]
        ...
        points_attn_mask = \
            points_invalid[:, None, None, :].repeat((1, self.num_heads, self.num_queries, 1)).flatten(0, 1)
        ...
        for i in range(self.num_transformer_decoder_layers):
            ...
            # attn_mask: batch_size * num_head, num_queries, (h*w + num_points)
            attn_masks = [torch.cat([attn_mask, points_attn_mask], dim=2), None]

            query_feat = layer(
                query=query_feat,
                key=torch.cat([decoder_inputs[level_idx], query_points], dim=0),
                value=torch.cat([decoder_inputs[level_idx], query_points], dim=0),
                query_pos=query_embed,
                key_pos=torch.cat([decoder_positional_encodings[level_idx], point_query_embed], dim=0),
                attn_masks=attn_masks,
                query_key_padding_mask=None,
                # here we do not apply masking on padded region
                key_padding_mask=None)
            cls_pred, mask_pred, attn_mask, iou_pred, onehot_pred = self.forward_head(
                query_feat,
                mask_features,
        ...
        return (torch.stack(cls_pred_list), torch.stack(mask_pred_list),
                torch.stack(iou_pred_list), torch.stack(onehot_pred_list))
